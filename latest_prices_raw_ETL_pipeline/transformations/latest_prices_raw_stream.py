...
# Create a Delta table sink for writing data to '{catalog}.01_bronze.latest_prices_raw'
dp.create_sink(
  name = "delta_sink",
  format = "delta",
  options = { "tableName": f"{catalog}.01_bronze.latest_prices_raw" }
)

# create path and schema for table
path = f"/Volumes/{catalog}/00_landing/data_sources/latest_prices_transformed/"
...
# write data to sink with append flow, writing data to '{catalog}.01_bronze.latest_prices_raw'
