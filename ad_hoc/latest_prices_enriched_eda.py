...
df2 = df.filter("id IN ('1161')") #Adamant full helm
df2 = df2.filter("time > 1769616000") # exclude older data
...
# std of buy prices
df2.filter("highorlow = 'high'").select(std("price")).show()

# COMMAND ----------

# std of sell prices
df2.filter("highorlow = 'low'").select(std("price")).show()
...
nature_rune_cost = 100

df3 = df.filter("time > 1769029986") # exclude older data
df3 = df3.withColumn("alch_result", col("highalch") - lit(nature_rune_cost) - col("price"))
df3 = df3.sort("alch_result", ascending=False)
df3.display()

# COMMAND ----------

df4 = df.filter("id IN ('1161','1731','1725','1199','1073')")
df4 = df4.filter("time > 1769701000") # exclude older data
df4 = df4.sort("NYCTime", ascending=False)
df4.display()

# COMMAND ----------

df5 = df4.groupBy("id","highorlow").agg(round(avg("price"), 2).alias("avg_price"), round(std("price"), 2).alias("std_price"))
df5 = df5.sort("id")
df5.display()
...
# TODO add a column for highalch profits
df8 = df7.filter("highorlow = 'high'").sort("std/avg", ascending=False)
#df8.filter("std_price >= .5")
df8.display()
